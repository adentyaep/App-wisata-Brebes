...
@section ('content')
    <div class="container mt-5">
        <div class="mb-2">
            <h2>Form Edit Data Restoran</h2>
        </div>

        <form action="/restoran/update/{{$data->id}}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
                <label for="formGroupExampleInput" class="form-label">Nama Restoran</label>
                <input type="text" class="form-control" name="frNamaRestoran" value="{{ old('nama_restoran') ? old('nama_restoran') : $data->nama_restoran }}">
            </div>
            <div class="mb-3">
